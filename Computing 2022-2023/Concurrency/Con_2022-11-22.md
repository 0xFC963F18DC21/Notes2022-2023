...
- Unlocking the mutex:
    - Store 0 to `state`.
    - Call `FUTEX_WAIT` in case other threads are waiting.
    - Ask `FUTEX_WAIT` to wake up one waiter (this is safe).
...
... however, this design is rather deficient, as `FUTEX_WAIT` is used on all unlocks, and this is wasteful if there are no waiters.
